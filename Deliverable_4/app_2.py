...
with tab2:
   ...
   if selected_symbol == "ETHEREUM":
       ...
       pred_plot = go.Figure()
       pred_plot.add_trace(go.Scatter(x=filtered_df["event_time"], y=filtered_df["prediction"], name="Predicted", line=dict(color="#FF6B6B")))
       pred_plot.add_trace(go.Scatter(x=filtered_df["event_time"], y=filtered_df["label"], name="Expected", line=dict(color="#4ECDC4")))
       pred_plot.add_trace(go.Scatter(x=filtered_df["event_time"], y=filtered_df["actual_price"], name="Actual", line=dict(color="#45B7D1")))
       ...
       pred_plot = px.line(filtered_df, x="event_time", y="prediction", title=f"Price Prediction vs Actual for {selected_symbol}")
       label_plot = px.line(filtered_df[filtered_df["label"].notna()], x="event_time", y="label")
       actual_plot = px.line(actual_df, x="event_time", y="actual_price")
