...
#             # Send to the original topic (test-topic)
#             self.kafka_producer.produce(
#                 self.kafka_topic, key=data["symbol"], value=json_string, callback=self.delivery_report
#             )
        
#             # Send to the additional topic (model-topic)
#             self.kafka_producer.produce(
#                 "model-topic", key=data["symbol"], value=json_string, callback=self.delivery_report
#             )

#             self.logger.debug(f"Sent to Kafka topics: {self.kafka_topic}, model-topic: {json_string}")
#             # Do not flush here; handle asynchronously
#         except Exception as e:
...
#         """Connect to the WebSocket and log in"""
#         try:
#             self.websocket = await websockets.connect(self.ws_url, ssl=self.ssl_context)
#             await self.login()
...
#                     "appId": "12345",  # Example App ID; replace as needed
...
#                 # Handle messages with 'returnData' and 'symbol'
...
#         # Flush Kafka producer before exiting
#         self.kafka_producer.flush()

# async def main():
#     # Replace with your XTB credentials and Kafka settings
#     client = XTBWebSocket(
#         userId="17051761",
#         password="PotezneBigData997",
#         kafka_bootstrap_servers="localhost:9092",  # Replace with your Kafka bootstrap server
#         kafka_topic="test-topic",  # Replace with your Kafka topic
#     )
